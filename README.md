...
Phase 3B適時開示基盤、無料取得自動化基盤、戦略タグ・共通ルール機能はv0.5.0 Candidateです。

Phase 4A企業カルテ・テーマカテゴリ管理機能はv0.6.0 Candidateです。

Phase 4B投資ルール機能・オレ株スコア機能はv0.7.0 Candidateです。

Phase 4C戦略タグ・共通ルール機能とFree Automation FoundationもCandidateです。実データ、実画面、Windowsタスクスケジューラ、EDINET実通信などの確認が完了するまで正式版へ昇格しません。

## 開発運用

GitHubリポジトリを唯一の正本とし、`main`を基準ブランチにします。自動開発はGitHubのopen issue、open PR、`EXECUTION_TASK.json`、`ROADMAP.md`を確認して、検証可能な小さいタスクを1件ずつ選びます。実装後はテスト、コミット、push、GitHub状態更新まで行います。人間確認やAPIキーが必要な作業はCandidateまたはblockerとして記録し、投資判断の断定、自動売買、注文発注は行いません。詳細は[AUTONOMOUS_DEVELOPMENT.md](AUTONOMOUS_DEVELOPMENT.md)を参照してください。
