...
現時点の無料adapterは楽天証券MarketSpeed形式を対象に、UTF-8 BOM / UTF-8 / CP932の判定、ticker正規化、口座別保有数量の集約、preview、既存銘柄との比較を行います。`services/securities_csv.py` がprovider dispatchの入口となり、MarketSpeedは既存parserへ委譲します。
...
- provider dispatchで未提供形式をfail-closedにし、列名推測や他社形式への暗黙fallbackを行わない。
